			     ...
			     const struct argparse_option *opt, int flags)
{
	...
	case ARGPARSE_OPT_INTEGER:
		...
		if (self->optvalue) {
			*(int *)opt->value =
			    strtol(self->optvalue, (char **)&s, 0);
			self->optvalue = NULL;
		} else if (self->argc > 1) {
			self->argc--;
			*(int *)opt->value =
			    strtol(*++self->argv, (char **)&s, 0);
		} else {
			argparse_error(self, opt, "requires a value", flags);
		}
		if (errno == ERANGE)
			argparse_error(self, opt,
				       "numerical result out of range", flags);
	...
	case ARGPARSE_OPT_FLOAT:
		...
		if (self->optvalue) {
			*(float *)opt->value =
			    strtof(self->optvalue, (char **)&s);
			self->optvalue = NULL;
		} else if (self->argc > 1) {
			self->argc--;
			*(float *)opt->value =
			    strtof(*++self->argv, (char **)&s);
		} else {
			argparse_error(self, opt, "requires a value", flags);
		}
		if (errno == ERANGE)
			      ...
			      const struct argparse_option *options)
{
	for (; options->type != ARGPARSE_OPT_END; options++) {
		if (options->short_name == *self->optvalue) {
			self->optvalue =
			    self->optvalue[1] ? self->optvalue + 1 : NULL;
			     ...
			     const struct argparse_option *options)
{
	for (; options->type != ARGPARSE_OPT_END; options++) {
		...
		if (!rest) {
			...
			rest = prefix_skip(self->argv[0] + 2 + 3,
					   options->long_name);
			if (!rest)
				continue;
			opt_flags |= OPT_UNSET;
...
int argparse_parse(struct argparse *self, int argc, const char **argv)
{
	...
	for (; self->argc; self->argc--, self->argv++) {
		...
		if (arg[0] != '-' || !arg[1]) {
			...
			// if it's not option or is a single char '-', copy
			// verbatim
